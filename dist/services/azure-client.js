...
const logger = {
    info: (msg, ctx) => console.error(`INFO: ${msg}`, ctx || ''),
    error: (msg, ctx) => console.error(`ERROR: ${msg}`, ctx || ''),
    warn: (msg, ctx) => console.error(`WARN: ${msg}`, ctx || ''),
    debug: (msg, ctx) => console.error(`DEBUG: ${msg}`, ctx || '')
};
/**
...
export class AzureClientManager {
    ...
    initializeClients() {
        try {
            ...
            this.credential = new ClientSecretCredential(this.config.tenantId, this.config.clientId, this.config.clientSecret, options);
            this.computeClient = new ComputeManagementClient(this.credential, this.config.subscriptionId);
            this.resourceClient = new ResourceManagementClient(this.credential, this.config.subscriptionId);
            this.webClient = new WebSiteManagementClient(this.credential, this.config.subscriptionId);
            this.storageClient = new StorageManagementClient(this.credential, this.config.subscriptionId);
    ...
    async executeWithRetry(operation, context = {}) {
        ...
        for (let attempt = 1; attempt <= this.retryOptions.maxRetries + 1; attempt++) {
            try {
                ...
                const delay = Math.min(this.retryOptions.baseDelay * Math.pow(2, attempt - 1), this.retryOptions.maxDelay);
    ...
    isRetryableError(error) {
        ...
        return this.retryOptions.retryableErrors.some(retryableError => errorMessage.includes(retryableError.toLowerCase()) ||
     ...
     */
    async getAllComputeResources(context = {}) {
     ...
     */
    async getVirtualMachine(resourceGroupName, vmName, context = {}) {
        return this.executeWithRetry(async () => {
     ...
     */
    async getVirtualMachineInstanceView(resourceGroupName, vmName, context = {}) {
        return this.executeWithRetry(async () => {
     ...
     */
    async getAppService(resourceGroupName, siteName, context = {}) {
        return this.executeWithRetry(async () => {
     ...
     */
    async createResourceGroup(resourceGroupName, location, tags = {}, context = {}) {
        return this.executeWithRetry(async () => {
            ...
            return await this.resourceClient.resourceGroups.createOrUpdate(resourceGroupName, resourceGroupParams);
        }, { ...context, operation: 'createResourceGroup', resourceId: resourceGroupName });
    }
    /**
     * Create a virtual machine
     */
    async createVirtualMachine(resourceGroupName, vmName, parameters, context = {}) {
        return this.executeWithRetry(async () => {
            const poller = await this.computeClient.virtualMachines.beginCreateOrUpdate(resourceGroupName, vmName, parameters);
     ...
     */
    async createAppServicePlan(resourceGroupName, planName, location, sku, context = {}) {
        return this.executeWithRetry(async () => {
            ...
            return await this.webClient.appServicePlans.beginCreateOrUpdateAndWait(resourceGroupName, planName, planParameters);
        }, { ...context, operation: 'createAppServicePlan', resourceId: planName });
    }
    /**
     * Create an App Service (Web App)
     */
    async createAppService(resourceGroupName, siteName, serverFarmId, location, runtime, context = {}) {
        return this.executeWithRetry(async () => {
            ...
            return await this.webClient.webApps.beginCreateOrUpdateAndWait(resourceGroupName, siteName, siteParameters);
        }, { ...context, operation: 'createAppService', resourceId: siteName });
    }
    /**
     * Create a storage account
     */
    async createStorageAccount(resourceGroupName, accountName, location, sku, context = {}) {
        return this.executeWithRetry(async () => {
            const parameters = {
                sku: {
                    name: sku
            ...
            const poller = await this.storageClient.storageAccounts.beginCreate(resourceGroupName, accountName, parameters);
            return await poller.pollUntilDone();
     ...
     */
    async getStorageAccountKeys(resourceGroupName, accountName, context = {}) {
        return this.executeWithRetry(async () => {
            const keys = await this.storageClient.storageAccounts.listKeys(resourceGroupName, accountName);
     ...
     */
    async createBlobServiceClient(resourceGroupName, accountName, context = {}) {
